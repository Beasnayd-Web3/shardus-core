...
class TransactionQueue {
  ...
  addTransactionToNonceQueue(nonceQueueEntry: NonceQueueItem): { success: boolean; reason?: string } {
    try {
      ...
      if (queue == null || (Array.isArray(queue) && queue.length === 0)) {
        ...
        if (logFlags.debug)
          this.mainLogger.debug(
            `addinng new nonce tx: ${nonceQueueEntry.txId} ${nonceQueueEntry.accountId} with nonce ${nonceQueueEntry.nonce}`
          )
      ...
      this.seqLogger.info(
        `0x53455106 ${shardusGetTime()} tx:${
          nonceQueueEntry.txId
        } Note over ${NodeList.activeIdToPartition.get(Self.id)}: pause_nonceQ`
      )
  ...
  routeAndQueueAcceptedTransaction(
    ...
    try {
      ...
      try {
        ...
        if (txQueueEntry.globalModification === false && this.executeInOneShard) {
          ...
          for (let idx = 0; idx < txQueueEntry.executionGroup.length; idx++) {
            ...
            if (node.id === ourID) {
              ...
              this.seqLogger.info(
                `0x53455105 ${shardusGetTime()} tx:${txId} Note over ${NodeList.activeIdToPartition.get(
                  Self.id
                )}: index ${txQueueEntry.ourExGroupIndex}:${(node as Shardus.NodeWithRank).rank}`
              )
            }
          }
                    ...
                    if (
                      ...
                      if (logFlags.seqdiagram) {
                        for (const node of this.stateManager.currentCycleShardData.syncingNeighborsTxGroup) {
                          this.seqLogger.info(
                            `0x53455102 ${shardusGetTime()} tx:${
                              acceptedTx.txId
                            } ${NodeList.activeIdToPartition.get(
                              Self.id
                            )}-->>${NodeList.activeIdToPartition.get(
                              node.id
                            )}: ${'spread_tx_to_group_syncing'}`
        ...
        if (logFlags.seqdiagram)
          this.seqLogger.info(
            `0x53455105 ${shardusGetTime()} tx:${
              txQueueEntry.acceptedTx.txId
            } Note over ${NodeList.activeIdToPartition.get(Self.id)}: pendingQ`
          )
      ...
      if (logFlags.debug || this.stateManager.consensusLog) {
        this.mainLogger.debug(
          `shareCompleteDataToNeighbours: shared complete data for txId ${
            queueEntry.logID
          } at timestamp: ${shardusGetTime()} nodeId: ${Self.id} to neighbours: ${JSON.stringify(
              ...
              if (logFlags.seqdiagram)
                this.seqLogger.info(
                  `0x53455101 ${shardusGetTime()} tx:${message.txid} ${NodeList.activeIdToPartition.get(
                    Self.id
                  )}-->>${NodeList.activeIdToPartition.get(node.id)}: ${'request_state_for_tx'}`
                )
          ...
          try {
            if (logFlags.seqdiagram)
              this.seqLogger.info(
                `0x53455101 ${shardusGetTime()} tx:${message.txid} ${NodeList.activeIdToPartition.get(
                  Self.id
                )}-->>${NodeList.activeIdToPartition.get(node.id)}: ${'request_receipt_for_tx'}`
              )
      ...
      if (logFlags.seqdiagram) {
        for (const node of nodes) {
          if (context == 'tellCorrespondingNodes') {
            this.seqLogger.info(
              `0x53455102 ${shardusGetTime()} tx:${message.txid} ${NodeList.activeIdToPartition.get(
                Self.id
              )}-->>${NodeList.activeIdToPartition.get(node.id)}: ${'broadcast_state_nodes'}`
            )
          } else {
            this.seqLogger.info(
              `0x53455102 ${shardusGetTime()} tx:${message.txid} ${NodeList.activeIdToPartition.get(
                Self.id
              )}-->>${NodeList.activeIdToPartition.get(node.id)}: ${'broadcast_state_neighbour'}`
            )
          }
        }
      }
            ...
            if (logFlags.seqdiagram) {
              for (const node of filterdCorrespondingAccNodes) {
                this.seqLogger.info(
                  `0x53455102 ${shardusGetTime()} tx:${message.txid} ${NodeList.activeIdToPartition.get(
                    Self.id
                  )}-->>${NodeList.activeIdToPartition.get(node.id)}: ${'broadcast_finalstate'}`
                )
              }
            }
  ...
  removeFromQueue(queueEntry: QueueEntry, currentIndex: number, archive = true): void {
    ...
    if (logFlags.seqdiagram)
      this.seqLogger.info(
        `0x53455104 ${shardusGetTime()} tx:${
          queueEntry.acceptedTx.txId
        } Note over ${NodeList.activeIdToPartition.get(Self.id)}: removed`
      )
        ...
        for (const txQueueEntry of this.pendingTransactionQueue) {
          ...
          if (logFlags.seqdiagram)
            this.seqLogger.info(
              `0x53455105 ${shardusGetTime()} tx:${
                txQueueEntry.acceptedTx.txId
              } Note over ${NodeList.activeIdToPartition.get(Self.id)}: aging`
            )
  ...
  async requestFinalData(queueEntry: QueueEntry, accountIds: string[]) {
    ...
    try {
      ...
      if (this.config.p2p.useBinarySerializedEndpoints && this.config.p2p.requestTxAndStateBinary) {
        ...
        if (logFlags.seqdiagram)
          this.seqLogger.info(
            `0x53455101 ${shardusGetTime()} tx:${
              queueEntry.acceptedTx.txId
            } ${NodeList.activeIdToPartition.get(Self.id)}-->>${NodeList.activeIdToPartition.get(
              nodeToAsk.id
    ...
    if (logFlags.seqdiagram)
      if (context == '')
        this.seqLogger.info(
          `0x53455104 ${shardusGetTime()} tx:${
            queueEntry.acceptedTx.txId
          } Note over ${NodeList.activeIdToPartition.get(Self.id)}: ${queueEntry.state}-${nextState}`
        )
      else
        this.seqLogger.info(
          `0x53455104 ${shardusGetTime()} tx:${
            queueEntry.acceptedTx.txId
          } Note over ${NodeList.activeIdToPartition.get(Self.id)}: ${
            queueEntry.state
          }-${nextState}:${context}`
        )
